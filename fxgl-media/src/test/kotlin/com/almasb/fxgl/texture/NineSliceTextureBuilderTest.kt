...
class NineSliceTextureBuilderTest {
  ...
  private fun buildAndCompareTexture(targetTexture: Texture) {
    val shrunkBaseTexture = builder.build(targetTexture.width.toInt(), targetTexture.height.toInt())

    MatcherAssert.assertThat(shrunkBaseTexture.width, CoreMatchers.`is`(targetTexture.width))
    MatcherAssert.assertThat(shrunkBaseTexture.height, CoreMatchers.`is`(targetTexture.height))

    MatcherAssert.assertThat("Colors of shrunken texture does not match with expected!", matchPixels(shrunkBaseTexture, targetTexture))
  }
